...
export class AuthenticationService {
  ...
  public login(body: object) {
    return this.http.post('/api/user/login', body, jsonHeader()).map(this.extractToken).catch(this.handleError);
  }

  public address(body: object){
    return this.http.post('/api/user/wallet', body, jsonHeader())
  }
  ...
  public register(body: object) {
    return this.http.post('api/user/registers', body, jsonHeader()).map(this.extractToken).catch(this.handleError);
  }
  ...
  public logout() {
    if (this.isAuthenticated()) {
      this.postResource('', '/api/user/logout')
   ...
   * @param {Object} body
   */
  public postJSON(body: object, url: string) {
